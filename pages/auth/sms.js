...
Page({
  ...
  toggleShow(){
    ...
    this.setData({alowTerms:show})
},
sendYanzheng(){
  var _this= this
  var phone = this.data.phone
  if(!phone ){
    wx.showModal({title: '手机号不能为空',icon: 'none'})
    return false
  }
  ...
  Timeout(){
    ...
    setTimeout(() => {
        ...
        if(this.data.time<=0){
            this.setData({time:60,yanzhengShow:false})
            return
        }else{
            this.Timeout()
        }
  ...
  loginHandle(){
    ...
    if(yanzheng.length != 4){
      wx.showModal({title: '验证码输入错误',icon: 'none'})
      return false
    }
    app.request({
      url:'/api/v1/sms/auth',
